...
var gfillGreaterThanEqual = require( './../lib' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof gfillGreaterThanEqual, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function replaces elements greater than or equal to a provided search element', function test( t ) {
	var searchElement;
	var expected;
	var actual;
	var alpha;
	...
	actual = gfillGreaterThanEqual( [ x, searchElement, alpha, start, end ] );
	t.strictEqual( actual, x, 'returns expected value' );

	expected = [ 0.0, -2.0, 5.0, 0.0, 5.0, -6.0 ];
...
tape( 'the function supports an input ndarray having a non-zero offset', function test( t ) {
	...
	expected = [ 0.0, -2.0, 3.0, 0.0, 5.0, -6.0 ];
	t.deepEqual( xbuf, expected, 'returns expected value' );
...
tape( 'if all elements are not greater than or equal to a provided search element, the function returns the input ndarray unchanged', function test( t ) {
	var searchElement;
	var expected;
	var actual;
	var alpha;
	...
	x = vector( xbuf, 4, 1, 0 );
	searchElement = scalar2ndarray( 5.0, {
		'dtype': 'generic'
	});
	alpha = scalar2ndarray( 5.0, {
		'dtype': 'generic'
...
tape( 'if provided a search element equal to `NaN`, the function returns the input ndarray unchanged', function test( t ) {
	...
	t.strictEqual( isSameArray( xbuf, expected ), true, 'returns expected value' );
